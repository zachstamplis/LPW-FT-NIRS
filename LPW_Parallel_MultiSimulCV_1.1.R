...
# previous parameters, outliers removed, 1000, ALL WAVES 

# MAC GAVE ME THIS
# best_params_xgb <- data.frame(nrounds = 500, max_depth = 2, eta = 0.1, gamma = 0, colsample_bytree = 0.8, min_child_weight = 8, subsample = 1)
...
extract_all_predictions <- function(model_results, split_set) {
  ...
  for (model_type in names(model_results)) {
    ...
    for (fold in seq_along(pred_list)) {
      ...
      base_df <- data.frame(split_set = split_set, fold = fold, specimen_number = fold_preds$specimen_number, actual = fold_preds$actual)
      if (model_type %in% c("lm", "gam")) {
        for (variant in 1:10) {
          variant_df <- base_df; variant_df$model_type <- toupper(model_type); variant_df$model_variant <- paste(toupper(model_type), variant)
          variant_df$predicted <- fold_preds$model_preds[[variant]]; variant_df$components <- fold_preds$model_comps[[variant]]
          all_preds[[length(all_preds) + 1]] <- variant_df
        }
      } else if (model_type == "pls") {
        pls_df <- base_df; pls_df$model_type <- "PLS"; pls_df$model_variant <- "PLS"; pls_df$predicted <- fold_preds$pls_pred; pls_df$components <- fold_preds$pls_ncomp
        all_preds[[length(all_preds) + 1]] <- pls_df
        vip_df <- base_df; vip_df$model_type <- "PLS"; vip_df$model_variant <- "PLS-VIP"; vip_df$predicted <- fold_preds$vip_pred; vip_df$components <- fold_preds$vip_ncomp
        all_preds[[length(all_preds) + 1]] <- vip_df
      } else if (model_type == "rf") {
        rf_df <- base_df; rf_df$model_type <- "RF"; rf_df$model_variant <- "RF"; rf_df$predicted <- fold_preds$rf_pred; rf_df$components <- fold_preds$components
        all_preds[[length(all_preds) + 1]] <- rf_df
      } else if (model_type == "xgb") {
        xgb_df <- base_df; xgb_df$model_type <- "XGB"; xgb_df$model_variant <- "XGBoost"; xgb_df$predicted <- fold_preds$xgb_pred; xgb_df$components <- fold_preds$components
...
# Running with outliers removed, 500 iterations, LOOCV, 1000 stopping rounds for XGBoost (didn't reference best_params_xgb$nrounds yet, will run another WITH 2000 as selected by my PC) 
...
saveRDS(all_results_means_final, paste0("RDS_dataframes/all_results_means_parallel_LOOCV_1000_", Sys.Date(), ".RDS"))
saveRDS(all_predictions_final, paste0("RDS_dataframes/all_predictions_parallel_LOOCV_1000_", Sys.Date(), ".RDS"))
saveRDS(final_importance_data, paste0("RDS_dataframes/final_importance_data_parallel_LOOCV_1000_", Sys.Date(), ".RDS"))
