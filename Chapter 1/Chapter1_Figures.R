...
# p1.min <- ggplot(pca_data, aes(x = wavenumber)) +
#   geom_ribbon(aes(ymin = min, ymax = max), fill = "#4477AA", alpha = 0.5) +
#   geom_line(aes(y = final_mean), color = "#4477AA") +
#   labs(y = "PCA\nLoadings", x = NULL) +
#   theme_bw(base_size = 15) +
#   theme(axis.text.x = element_blank(),
...
# p2.min <- ggplot(pls_data, aes(x = wavenumber)) +
#   geom_ribbon(aes(ymin = min, ymax = max), fill = "#AA3377", alpha = 0.5) +
#   geom_line(aes(y = final_mean), color = "#AA3377") +
#   labs(y = "VIP Score\n(PLS)", x = NULL) +
#   theme_bw(base_size = 15) +
#   theme(axis.text.x = element_blank(),
...
p3 <- ggplot(xgb_data, aes(x = wavenumber)) +
  ...
  theme(axis.text.x = element_blank(),
        axis.ticks.x = element_blank(),
        axis.title.y = element_text(angle = 90, vjust = 0.5))
# p3.min <- ggplot(xgb_data, aes(x = wavenumber)) +
#   geom_ribbon(aes(ymin = min, ymax = max), fill = "#228833", alpha = 0.5) +
#   geom_line(aes(y = final_mean), color = "#228833") +
#   labs(y = "Gain\n(XGB)", x = NULL) +
#   theme_bw(base_size = 15) +
#   theme(axis.text.x = element_blank(),
...
# p4.min <- ggplot(rf_data, aes(x = wavenumber)) +
#   geom_ribbon(aes(ymin = min, ymax = max), fill = "#228833", alpha = 0.5) +
#   geom_line(aes(y = final_mean), color = "#228833") +
#   labs(y = "Permutation\n(RF)", x = expression(paste("Wavenumber (cm"^{-1}, ")"))) +
#   theme_bw(base_size = 15) +
...
final_plot <- (p1 / p2 / p3 / p4) & scale_x_reverse(expand = expansion(mult = 0, add = 50))

final_plot

ggsave(
  filename = paste0("Model Results/", "LPW", "_waveimportance_range_", timestamp, ".png"), # Desired file name
  plot = final_plot,                              # The plot object to save
...
# # Combine the four plots vertically
# final_plot <- (p1.min / p2.min / p3.min / p4.min) & scale_x_reverse(expand = expansion(mult = 0, add = 50))
# 
# final_plot
# 
# ggsave(
#   filename = "waveimportance_range.png", # Desired file name
#   plot = final_plot,                              # The plot object to save
...
# Assuming 'p1', 'p2', 'p3', and 'p4' are the original ggplot objects, correctly defined 
# with their respective importance data and scales (as in your prompt).
...
# The plots p1, p2, p3, p4 must be defined from your importance data summary 
# before running this step.

p1_final <- add_spectra_overlay(p1, pca_data) + theme(
  # ... (other theme elements)
  axis.title.x = element_blank(), 
  axis.text.x = element_blank(), 
  axis.ticks.x = element_blank(),
  axis.line.x = element_blank(), # Ensure the axis line is also gone if present
  panel.grid.major = element_blank(), 
  panel.grid.minor = element_blank()
)
p2_final <- add_spectra_overlay(p2, pls_data) + theme(
  # ... (other theme elements)
  axis.title.x = element_blank(), 
  axis.text.x = element_blank(), 
  axis.ticks.x = element_blank(),
...
p3_final <- add_spectra_overlay(p3, xgb_data) + theme(
  # ... (other theme elements)
  axis.title.x = element_blank(), 
  axis.text.x = element_blank(), 
  axis.ticks.x = element_blank(),
...
p4_final <- add_spectra_overlay(p4, rf_data) + theme(
  # ... (other theme elements)
  axis.title.x = element_text(size = 15), # Keep the X-axis title
  axis.text.x = element_text(size = 15), # Keep the X-axis labels
  axis.ticks.x = element_line(),         # Keep the X-axis ticks
...
# Use the same combination logic as your original successful setup
final_combined_plot_outline <- (p1_final / p2_final / p3_final / p4_final) & 
  scale_x_reverse(expand = expansion(mult = 0, add = 50)) # Apply shared X-axis settings
...
ggsave(
  filename = paste0("Model Results/", "LPW", "_waveimportance_overlay_", timestamp, ".png"), # Desired file name
  plot = final_combined_plot_outline,                              # The plot object to save
  width = 10,                                     # Width in inches (adjust as needed)
  height = 10,                                    # Height in inches (adjust as needed for 4 vertical plots)
  dpi = 600                                      # High resolution (300 is standard for print)
)
