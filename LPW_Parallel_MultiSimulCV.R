...
run_lm_models <- function(cal, test, terms_lm) {
  ...
  for (i in 1:10) {
    ...
    fold_preds <- list(specimen_number = testing$specimen, actual = testing$read_age, model_preds = vector("list", 10))
    for (j in 1:10) {
...
run_rf_models <- function(cal, test, best_params_rf) {
  ...
  for (i in 1:10) {
    calibrate <- cal[[i]]; testing <- test[[i]]
    fold_preds <- list(specimen_number = testing$specimen, actual = testing$read_age, rf_pred = numeric(length(testing$read_age)))
    fold_results <- data.frame(Fold = i, Model = "RF", R2 = numeric(1), RMSE = numeric(1), RPD = numeric(1), Bias = numeric(1), PercentRMSE = numeric(1))
    mod <- ranger(x = calibrate[, 31:ncol(calibrate)], y = calibrate$read_age, mtry = best_params_rf$mtry, min.node.size = best_params_rf$min.node.size, seed = 6, importance = 'permutation')
    wavenumbers <- as.numeric(colnames(calibrate[, 31:ncol(calibrate)])); importance_scores <- ranger::importance(mod)
