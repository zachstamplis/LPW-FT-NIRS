...
scan_avg <- NULL
scan_avg <- bind_cols(scan_avg, scan_2[, 1:22])
...
Initial preprocessing done with prospectr and a savitzkygolay filter.  might make more sense to preprocess after selecting important variables?  Some functions included to play around with some preprocessing filters and select optimal filters.
...
test_waves <- names(subset(test_vip[,1], test_vip[,1] > 2))
