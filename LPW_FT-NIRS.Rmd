...
quickproc(age_only,1,2,17)

spec.fig(temp_proc_long,specimen)

# test pls, no variable selection
test_pls_age <- pls(temp_proc[, 23:ncol(temp_proc)], temp_proc[, 13],
...
test_pls_age_VIP <- pls(temp_proc[, 23:ncol(temp_proc)], temp_proc[, 13],
 scale = FALSE, center = TRUE, info = "Age Prediction Model",
 cv = 1, exclcols = bad_waves
)
...
idx <- floor(nrow(temp_proc) * 0.9)
train_idx <- sample(seq_len(nrow(temp_proc)), size = idx)
train <- temp_proc[train_idx, -c(1:12,14:22)] 
test <- temp_proc[-train_idx, -c(1:12,14:22)] 
...
# 90/10 split for PLS, VIP > 0.5 (not working???)
...
```{r, cache = T}
pca_age_LPW <- pca(temp_proc[23:nrow(temp_proc)], scale = T)
mlr_age_pcs <- pca_age_LPW$calres$scores[,1:20]
mlr_age_df <- cbind(mlr_age_pcs[,1:10], temp_proc[,3:13])

set.seed(1)
idx <- floor(nrow(temp_proc) * 0.9)
train_idx <- sample(seq_len(nrow(temp_proc)), size = idx)
train <- temp_proc[train_idx, -c(1:12,14:33)] 
test <- temp_proc[-train_idx, -c(1:12,14:33)] 
