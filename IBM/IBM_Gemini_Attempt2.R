...
packages <- c("caret", "dplyr", "mgcv", "MuMIn", "purrr", "ranger", "stringr",
              "xgboost", "future", "furrr", "progressr", "mdatools", "cli", "tidyr", "ggplot2", "prospectr", "patchwork", "viridis", "tidyverse", "ggrepel")
...
# --- C. Global Parameters ---
N_SPLITS <- 20 # Number of different train/test data splits (e.g., 500)
...
df_full <- readRDS("RDS_dataframes/IBM_SGpreproc.RDS") %>% filter(run_number == 2)
...
metadata <- df[, !(names(df) %in% SPECTRAL_COLS)]
...
pca_results <- pca(spectral_data, scale = TRUE, alpha = 0.01, gamma = 0.01)

plotResiduals(pca_results)
pca_results$limParams
...
ggsave(
  filename = paste0("Model Results/", "IBM", "_PCA_outliers_", timestamp, ".png"),
  plot = PCA_outliers,
  device = "png",
  width = 7,
  height = 6,
...
BEST_PARAMS_XGB <- readRDS("Tuning/IBM_best_xgb_params_tuned_2025-10-28.RDS")
BEST_PARAMS_RF <- readRDS("Tuning/IBM_best_RF_params_tuned_2025-10-28.RDS")
...
run_pls_models_fast <- function(calibrate, testing) {
  mod_pls <- mdatools::pls(
    calibrate[, SPECTRAL_COLS],
    calibrate[, "read_age"],
    cv = 1,           # NO internal cross-validation# USE THE PRE-CALCULATED VALUE
    scale = FALSE, 
    center = TRUE,
...
run_analysis_for_fold <- function(job_row, all_data, ...) {
  ...
  # --- PLS Models ---
  if (RUN_PLS_MODELS) {
    pls_out <- run_pls_models_fast(calibrate_raw, testing_raw)
...
# }
# 
# saveRDS(job_manifest, paste0("Model Results/","job_manifest_", "IBM_", timestamp, ".RDS"))


# if (RUN_PLS_MODELS) {
#   cat("--- Pre-calculating optimal PLS components for", nrow(job_manifest), "folds... ---\n")
# 
#   find_optimal_ncomp <- function(train_idx, data, progress = NULL) {
#     calibrate <- data[train_idx, ]
#     mod <- mdatools::pls(
#       calibrate[, SPECTRAL_COLS],
#       calibrate[[RESPONSE_VAR]],
...
#       center = TRUE
#     )
#     if (!is.null(progress)) progress()  # Signal progress
#     return(mod$ncomp.selected)
#   }
# 
#   plan(multisession, workers = N_CORES_PREP)
...
# 
#   with_progress({
#     p <- progressor(steps = length(job_manifest$train_indices))
# 
#     optimal_ncomps <- future_map_dbl(
#       .x = job_manifest$train_indices,
#       .f = ~ find_optimal_ncomp(.x, data = df, progress = p),
#       .options = furrr_options(seed = TRUE, packages = "mdatools")
#     )
#   })
# 
...
#   find_optimal_ncomp <- function(train_idx, data) {
#     calibrate <- data[train_idx, ]
#     mod <- mdatools::pls(calibrate[, SPECTRAL_COLS], calibrate[[RESPONSE_VAR]], cv = 1, ncomp = 10, center = TRUE)
#     return(mod$ncomp.selected)
#   }
#   
#   plan(multisession, workers = N_CORES_PREP)
...
if (RUN_SIMPLE_MODELS) {
  ...
  process_single_split_simple <- function(split_num, data, splits_list) {
    ...
    map_dfr(1:10, function(fold_num) {
      ...
      # Simple linear model
      mod_lm <- lm(data = calibrate_data, read_age ~ length + structure_weight)
      preds_lm <- predict(mod_lm, newdata = testing_data)
      
      # Simple GAM model
      mod_gam <- gam(data = calibrate_data, read_age ~ s(length, k = 4) + s(structure_weight, k = 4), method = "REML")
...
saveRDS(final_results_summary, paste0("RDS_dataframes/", "IBM", "_SUMMARY_all_models_", timestamp, ".RDS"))
saveRDS(final_predictions_df, paste0("RDS_dataframes/", "IBM", "_PREDICTIONS_all_models_", timestamp, ".RDS"))
saveRDS(final_importance_summary, paste0("RDS_dataframes/", "IBM", "_IMPORTANCE_all_models_", timestamp, ".RDS"))
