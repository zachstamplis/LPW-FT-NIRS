...
saveRDS(dfmeta_LPW, "RDS_dataframes/LPW_dfmeta_RAW_allscans.RDS")
```
...
# preprocess - AFTER TAKING AVERAGE (maybe incorrect here??)
...
# scan_avg_proc <- readRDS("RDS_dataframes/LPW_scan_avg_proc.RDS")
...
plotly::ggplotly(
  ggplot(data = scan_avg_filter, aes(x = PC1, y = PC2, label = specimen, color = read_age)) +
    geom_point(size = 3) + 
    scale_color_viridis())

plotly::ggplotly(
  ggplot(data = scan_avg_filter, aes(x = specimen, y = read_age, label = specimen, color = structure_weight)) +
    geom_point(size = 3) + 
    scale_color_viridis() + 
  geom_smooth(method = "lm"))

plotly::ggplotly(
  ggplot(data = scan_avg_filter, aes(x = length, y = structure_weight, label = specimen, color = read_age)) +
    geom_point(size = 3) + 
    scale_color_viridis() + 
  geom_smooth(method = "lm"))

plotly::ggplotly(
  ggplot(data = scan_avg_filter, aes(x = structure_weight, y = read_age, label = specimen, color = length)) +
    geom_point(size = 3) + 
    scale_color_viridis() + 
  geom_smooth(method = "lm"))
```
...
for (i in 1:10) {
  pctest <- cal[[i]]
  temp <- step(lm(data = pctest[-splits[[i]], ], read_age ~ PC1 + PC2 + PC3 + PC4),trace = 0)
...
```{r}
knitr::includeHTML("plots/shiny_plot.html")
```
