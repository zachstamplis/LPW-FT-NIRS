...
df <- df %>% filter(specimen != 53
                    # , specimen != 74, no longer an outlier based on 99% t2 confidence ellipse
                    ) # remove outliers
...
# PULL PC'S FOR USE IN TABLE 
# # Assuming 'dredge_gam' and 'dredge_lm' are your MuMIn::dredge output objects
# # and that they have the same PC variables in their global models.
# 
# # --- Extract PCs for GAM Models ---
# # 1. Take the top 10 models from the dredge result.
# # 2. Select only the columns corresponding to the smoothed PC terms (s(PCn, 4)).
# # 3. Use 'apply' to loop through the rows and concatenate the PC names.
# gam_pcs <- head(dredge_gam, 10)
# 
# # Identify columns that start with "s(PC"
# gam_pc_cols <- grep("^s\\(PC", names(gam_pcs))
# 
# # Extract the PC number from the column names, e.g., "s(PC1,4)" -> "PC1"
# gam_pc_names <- sub("s\\((PC[0-9]+).*", "\\1", names(gam_pcs)[gam_pc_cols])
# 
# # Apply a function to each row (MARGIN = 1) to build the PC string
# gam_pc_list <- apply(gam_pcs[, gam_pc_cols], 1, function(row) {
#   # 'row == "+"' identifies which PCs were included (marked with '+')
#   included_pcs <- gam_pc_names[row == "+"]
#   # Collapse the names into a single, comma-separated string
#   paste(included_pcs, collapse = ", ")
# })
# 
# # Create the summary table for GAM
# gam_summary_table <- data.frame(
#   Model_Rank = 1:10,
#   AICc = round(gam_pcs$AICc, 2),
#   PCs = gam_pc_list
# )
# 
# 
# # --- Extract PCs for LM Models ---
# # 1. Take the top 10 models from the dredge result.
# # 2. Select only the columns corresponding to the raw PC terms (PCn).
# # 3. Use 'apply' to loop through the rows and concatenate the PC names.
# lm_pcs <- head(dredge_lm, 10)
# 
# # Identify columns that are exactly "PC" followed by a number
# lm_pc_cols <- grep("^PC[0-9]+$", names(lm_pcs))
# 
# # Extract the column names (which are already the PC names)
# lm_pc_names <- names(lm_pcs)[lm_pc_cols]
# 
# # Apply a function to each row (MARGIN = 1) to build the PC string
# lm_pc_list <- apply(lm_pcs[, lm_pc_cols], 1, function(row) {
#   # 'is.na(row)' is TRUE if the term was NOT included (blank in dredge output)
#   included_pcs <- lm_pc_names[!is.na(row)]
#   # Collapse the names into a single, comma-separated string
#   paste(included_pcs, collapse = ", ")
# })
# 
# # Create the summary table for LM
# lm_summary_table <- data.frame(
#   Model_Rank = 1:10,
#   AICc = round(lm_pcs$AICc, 2),
#   PCs = lm_pc_list
# )
# 
# # --- Print Results ---
# cat("============================================\n")
# cat("   Top 10 GAM Models: Included Principal Components\n")
# cat("============================================\n")
# print(gam_summary_table)
# 
# cat("\n============================================\n")
# cat("   Top 10 LM Models: Included Principal Components\n")
# cat("============================================\n")
...
calculate_pca_vector_length <- function(pca_rotation, pc_indices) {
  # Subset the loadings for the PCs used in the model
  loadings_subset <- pca_rotation[, pc_indices, drop = FALSE]
  
  # Square the loadings, sum them row-wise, and take the square root
...
best_params_xgb <- data.frame(colsample_bytree = 0.8, eta = 0.05,  gamma = 1, max_depth = 6,   min_child_weight = 8, subsample = 1)
...
n_splits <- 500 # Changed from 500 to 50 as per user's test
set.seed(6)
...
#> print(end_time_stage1 - start_time_stage1)
# Time difference of 28.75257 mins on the second run 10/20/2025
  ...
  map_dfr(1:10, function(fold_num) {
    ...
    bind_rows(
      ...
      mutate(
        Split = split_num,
        Fold = fold_num,
        specimen_number = rep(testing_data$specimen, 2), # Corrected line
        actual = rep(testing_data$read_age, 2)          # Corrected line
      )
  })
}
...
saveRDS(all_results_means_final, paste0("RDS_dataframes/all_results_means_USEME_NEW_", Sys.Date(), ".RDS"))
saveRDS(all_predictions_final, paste0("RDS_dataframes/all_predictions_USEME_NEW_", Sys.Date(), ".RDS"))
saveRDS(final_importance_summary, paste0("RDS_dataframes/final_importance_data_USEME_NEW_", Sys.Date(), ".RDS"))
