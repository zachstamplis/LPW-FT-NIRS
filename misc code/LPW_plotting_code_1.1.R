...
# Updated method order to include 6 panels
...
# # 1. Convert to data.table format
# dt_importance <- data.table::as.data.table(final_importance_data)
# 
# # 2. Perform the aggregation using data.table syntax
# # Syntax: DT[i, j, by]
# # i = filters (none needed here)
# # j = calculations (your summarizes)
# # by = groups (your group_by variables)
# 
# importance_summarized_dt <- dt_importance[, 
#                                           .(
#                                             final_mean = mean(importance),
...
#                                           ), 
#                                           by = .(method, wavenumber, SplitSet)
# ]
# 
# importance_summarized_df <- as.data.frame(importance_summarized_dt)
# 
# saveRDS(importance_summarized_df, "RDS_dataframes/importance_summarized_10202025")
...
importance_summary <- all_importance %>%
  mutate(
    method = case_when(
      method == "PCA-LM-Wgt" ~ paste0("Loadings (", best_lm, ")"),
      method == "PCA-GAM-Wgt" ~ paste0("Loadings (", best_gam, ")"),
  ...
  summarise(
    ...
    lower_bound = min(lower_CI, na.rm = TRUE),
    upper_bound = max(upper_CI, na.rm = TRUE),
    lower_CI = quantile(mean_importance, 0.025, na.rm = TRUE),
    upper_CI = quantile(mean_importance, 0.975, na.rm = TRUE),
    .groups = "drop"
  )
...
importance_summary <- final_importance_data %>%
  # 1. Standardize column names (since you have both final_mean and mean_importance)
  # Assuming 'mean_importance' is the correct mean of the 10 folds.
  mutate(final_mean_split = coalesce(mean_importance, final_mean)) %>% 
  
  # 2. Filtering and re-labeling (No change needed here)
  mutate(
    method = case_when(
      method == paste0("PCA-LM", str_extract(best_lm, "\\d+")) ~ paste0("Loadings (", best_lm, ")"),
      method == paste0("PCA-GAM", str_extract(best_gam, "\\d+")) ~ paste0("Loadings (", best_gam, ")"),
      method == "PLS-VIP" ~ "VIP Score (PLS)",
      method == "XGBoost" ~ "Gain (XGB)",
      method == "Random Forest" ~ "Permutation (RF)",
      TRUE ~ as.character(method)
  ...
  # 3. Final Aggregation (Combining 500 SplitSets)
  group_by(method, wavenumber) %>%
  summarise(
    # FINAL MEAN: Average the 500 SplitSet means (final_mean_split)
    final_mean = mean(final_mean_split, na.rm = TRUE), 
    
    # GLOBAL MIN/MAX: Take the minimum of all 500 'lower_bound' values 
    # and maximum of all 500 'upper_bound' values to get the true full range.
    lower_bound = min(lower_bound, na.rm = TRUE),
    upper_bound = max(upper_bound, na.rm = TRUE),
    
    # 95% CI: Calculate the CI across the 500 SplitSet means (final_mean_split)
    lower_CI = quantile(final_mean_split, 0.025, na.rm = TRUE),
    upper_CI = quantile(final_mean_split, 0.975, na.rm = TRUE),
    .groups = "drop"
  )
...
# Create plots with full range ribbons
p1 <- ggplot(lm_data, aes(x = wavenumber)) +
  geom_ribbon(aes(ymin = lower_bound, ymax = upper_bound), fill = "#4477AA", alpha = 0.5) +
  geom_line(aes(y = final_mean), color = "#4477AA") +
  labs(y = paste0("Loadings\n(", best_lm, ")"), x = NULL) + 
...
# Create plots with 95% CI ribbons
p1_ci <- ggplot(lm_data, aes(x = wavenumber)) +
...
p2_ci <- ggplot(gam_data, aes(x = wavenumber)) +
  geom_ribbon(aes(ymin = lower_CI, ymax = upper_CI), fill = "#4477AA", alpha = 0.5) +
  geom_line(aes(y = final_mean), color = "#4477AA") +
  labs(y = paste0("Loadings\n(", best_gam, ")"), x = NULL) + 
  theme_bw() + 
...
p3_ci <- ggplot(pls_data, aes(x = wavenumber)) +
...
p4_ci <- ggplot(xgb_data, aes(x = wavenumber)) +
...
p5_ci <- ggplot(rf_data, aes(x = wavenumber)) +
  geom_ribbon(aes(ymin = lower_CI, ymax = upper_CI), fill = "#228833", alpha = 0.5) +
  geom_line(aes(y = final_mean), color = "#228833") +
  labs(y = "Permutation\n(RF)", x = expression(paste("Wavenumber (cm"^{-1}, ")"))) + 
  theme_bw() + 
...
# Combine plots with 95% CI
(p1_ci / p2_ci / p3_ci / p4_ci / p5_ci) +
  plot_annotation(
    title = "Wavenumber importance across model methods",
