# recommended at top of script, BEFORE parallel workers spawn
Sys.setenv(OMP_NUM_THREADS = "1")
Sys.setenv(MKL_NUM_THREADS = "1")
Sys.setenv(OPENBLAS_NUM_THREADS = "1")

# Also from R run-time control (RhpcBLASctl)
...
run_lm_models <- function(cal, test, terms_lm, pc_counts_lm) { # Added pc_counts_lm
  splits_results_lm <- data.frame()
  all_predictions <- list()
  for (i in 1:10) {
    calibrate <- cal[[i]]; testing <- test[[i]]
    fold_results <- data.frame(Fold = i, Model = paste0("Linear ", 1:10), R2 = numeric(10), RMSE = numeric(10), RPD = numeric(10), Bias = numeric(10), PercentRMSE = numeric(10), Components = numeric(10))
    fold_preds <- list(specimen_number = testing$specimen, actual = testing$read_age, model_preds = vector("list", 10), model_comps = vector("list", 10)) # Added model_comps
    for (j in 1:10) {
      ...
      RSS <- sum((testing$read_age - preds)^2); TSS <- sum((testing$read_age - mean(testing$read_age))^2)
...
################################################################################
run_gam_models <- function(cal, test, terms_gam, pc_counts_gam) { # Added pc_counts_gam
  splits_results_gam <- data.frame()
  all_predictions <- list()
  for (i in 1:10) {
    calibrate <- cal[[i]]; testing <- test[[i]]
    fold_results <- data.frame(Fold = i, Model = paste0("GAM ", 1:10), R2 = numeric(10), RMSE = numeric(10), RPD = numeric(10), Bias = numeric(10), PercentRMSE = numeric(10), Components = numeric(10))
    fold_preds <- list(specimen_number = testing$specimen, actual = testing$read_age, model_preds = vector("list", 10), model_comps = vector("list", 10)) # Added model_comps
    for (j in 1:10) {
...
run_pls_models <- function(cal, test) {
  ...
  for (i in 1:10) {
    ...
    fold_preds <- list(specimen_number = testing$specimen, actual = testing$read_age, pls_pred = numeric(length(testing$read_age)), vip_pred = numeric(length(testing$read_age)), pls_ncomp = NA_real_, vip_ncomp = NA_real_) # Added ncomp storage
    ...
    importance_df <- data.frame(fold = i, method = "PLS-VIP", wavenumber = wavenumbers, importance = vip_scores); all_importance_fold[[i]] <- importance_df
    ...
    fold_preds$pls_pred <- mod$testres$y.pred[, ncomp, ]; fold_preds$pls_ncomp <- ncomp # Store ncomp for predictions
    ...
    fold_preds$vip_pred <- mod$testres$y.pred[, ncomp, ]; fold_preds$vip_ncomp <- ncomp # Store ncomp for predictions
...
run_xgb_models <- function(cal, test, best_params_xgb) {
  ...
  for (i in 1:10) {
    ...
    params <- list(objective = "reg:squarederror", booster = "gbtree", eta = best_params_xgb$eta, max_depth = best_params_xgb$max_depth, min_child_weight = best_params_xgb$min_child_weight, subsample = best_params_xgb$subsample, gamma = best_params_xgb$gamma, colsample_bytree = best_params_xgb$colsample_bytree, nthread = 1) # Set nthread to 1
    ...
    fold_preds <- list(specimen_number = testing$specimen, actual = y_test, xgb_pred = preds, components = NA_real_) # Added components
    r2 <- cor(preds, y_test)^2; rmse_val <- sqrt(mean((preds - y_test)^2))
    fold_results <- data.frame(Fold = i, Model = "XGB", R2 = r2, RMSE = rmse_val, RPD = calculate_rpd(y_test, preds), Bias = calculate_bias(y_test, preds), PercentRMSE = rmse_val / max(y_test) * 100, Components = NA_real_) # Added components
...
process_single_split <- function(split_set, all_splits_data, df_data) {
  ...
  for (i in 1:10) {
    ...
    pc.mod <- preProcess(df_data[-splits[[i]], -c(1:20)], method = c("nzv", "center", "pca"), pcaComp = 10)
    
    cal[[i]] <- cbind(predict(pc.mod, df_data[-splits[[i]], -c(1:20)]), df_data[-splits[[i]], ])
    test[[i]] <- cbind(predict(pc.mod, df_data[splits[[i]], -c(1:20)]), df_data[splits[[i]], ])
...
################################################################################
# B. Configure and run the parallel job (clean version)
################################################################################

library(future)
library(future.apply)

# Number of repeated 10-fold CV sets
n_splits <- 10
set.seed(6)
all_splits <- generate_multiple_splits(df, n_splits)

# Disable nested parallelism inside caret/xgb/ranger
options(mc.cores = 1)
Sys.setenv(OMP_NUM_THREADS = 1)
Sys.setenv(MKL_NUM_THREADS = 1)
...
Sys.unsetenv("R_PARALLELLY_AVAILABLE_CORES")  # remove misleading limit if set
...
parallel_results_list <- future_lapply(
  1:n_splits, 
  FUN = function(i) {
    if (i %% 10 == 0) cat("Completed", i, "of", n_splits, "\n")
    process_single_split(i, all_splits_data = all_splits, df_data = df)
  },
  future.seed = TRUE,
  future.globals = list(
    process_single_split = process_single_split,
    ...
    run_lm_models = run_lm_models,
    run_gam_models = run_gam_models,
    run_pls_models = run_pls_models,
    run_rf_models = run_rf_models,
    run_xgb_models = run_xgb_models,
    extract_all_predictions = extract_all_predictions,
    calculate_rpd = calculate_rpd,
    calculate_bias = calculate_bias
  )
)
...
Sys.time()
  ...
  parallel_results_simple <- future_lapply(
    1:n_splits,
    FUN = function(i) process_single_split_simple(i, data = df_simple, splits_list = all_splits_simple),
...
saveRDS(all_results_means_final, paste0("RDS_dataframes/all_results_means_parallel_LOOCV_", Sys.Date(), ".RDS"))
