...
packages <- c("caret", "corrplot", "devtools", "DT", "dplyr", "gglm", "ggplot2", "gratia", "gridExtra", "knitr", "lubridate", "mdatools", "mgcv", "MuMIn", "opusreader2", "prospectr", "splitstackshape","shiny","shinyWidgets", "tidyr", "viridis")
...
df_filter <- df_filter %>% filter(specimen != 77)
...
global_gam_filter <- gam(data = pc_filter_df, read_age ~ s(PC1) + s(PC2) + s(PC3) + s(PC4) + s(PC5) + s(PC6))
dredge_lm_filter <- dredge(global_lm_filter)
dredge_gam_filter <- dredge(global_gam_filter)

# report differences in models selected
print("top 5 lm models all wavenumbers")
dredge_lm[1:5,]
print("top 5 lm models wavenumbers < 7500")
dredge_lm_filter[1:5,]
print("top 5 gam models all wavenumbers")
dredge_gam[1:5,]
print("top 5 gam models wavenumbers < 7500")
dredge_gam_filter[1:5,]
...
# Functions for above code to run all modelling with given data frame
...
saveRDS(results_allwaves, "shiny/results_allwaves.RDS")
saveRDS(mods_allwaves, "shiny/mods_allwaves.RDS")
```
...
saveRDS(results_filtered, "shiny/results_filtered.RDS")
saveRDS(mods_filtered, "shiny/mods_filtered.RDS")
```
...
saveRDS(results_allwaves_no77, "shiny/results_allwaves_no77.RDS")
saveRDS(mods_allwaves_no77, "shiny/mods_allwaves_no77.RDS")
```
